...
!  TKE, rate of dissipation, turbulent length-scale
   ...
   REALTYPE, public, dimension(:), allocatable   :: gotmss, gotmnn
...
!  allocate memory
   ...
   allocate(gotmss(0:nlev),stat=rc)
   if (rc /= 0) stop 'init_turbulence: Error allocating (gotmss)'
   gotmss = 0.d0
 
   allocate(gotmnn(0:nlev),stat=rc)
   if (rc /= 0) stop 'init_turbulence: Error allocating (gotmnn)'
   gotmnn = 0.d0

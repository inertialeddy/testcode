...
subroutine shearn2(ii,jj,step)

use header
implicit none

!!! Local Variables
integer :: ii,jj,k,step

double precision :: temp, dtr0 
...
if (step .eq. 1) then
  ...
  ush(ii,jj,NK) = stressx(jj)/(KzMom(ii,jj,NK) * R0 )
  vsh(ii,jj,NK) = stressy(jj)/(KzMom(ii,jj,NK) * R0 )
endif
...
do k=1, (NK-1)   
  nnface(ii,jj,k) = (1/DL)*(-9.81d0/R0) * (rho_old(ii,jj,k+1) - rho_old(ii,jj,k)) /  (zc(ii,jj,k+1) - zc(ii,jj,k))
...
k=NK   
  nnface(ii,jj,k) = (-9.81d0/R0)*( dtr0 * ( (1/KzTr(ii,jj,k))*( swr(jj) - qloss(jj) )/(R0*4187.d0) ) )

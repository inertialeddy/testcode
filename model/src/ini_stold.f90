    ...
    do i=0,NI+1 
      do k=0,NK+1 
        ...
        if (z.ge.-mldepth) then 
          Tbkgrnd =  seval(npl,-1.*mldepth,depoff,tvert,bT1,cT1,dT1) - (z+mldepth)*dtdz
          sbkgrnd =  seval(npl,-1.*mldepth,depoff,svert,bs1,cs1,ds1) - (z+mldepth)*dsdz
         else 
          Tbkgrnd = seval(npl,z,depoff,tvert,bT1,cT1,dT1)
          sbkgrnd = seval(npl,z,depoff,svert,bs1,cs1,ds1)        
        end if 
        T(i,j,k,0)=  Tbkgrnd 
        s(i,j,k,0)=  sbkgrnd 
      end do 

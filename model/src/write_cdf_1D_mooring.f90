      subroutine write_cdf_1D_mooring(imooring,jmooring,counter_1d) 
!     ------------------------------------------------------------      
                                                                        
!      implicit none                                                    
  USE header, ONLY : NI, NJ, NK, ntr, u, v, w, s, T, vor, rho, HL, h, dx, dy, zc, DL, time_seconds, dirout, rc_kind
...
#include "netcdf.inc" 
      ...
      REAL(kind=4) zprof(NK),n2prof(NK),uprof(NK),vprof(NK),wprof(NK),     &
           ugeoprof(NK),vgeoprof(NK),vorprof(NK),rhoprof(NK),sprof(NK),    &
           Tprof(NK)
...
!         idvhxy = ncvdef(idDatFile,'hxy',NCFLOAT,1,dims(2),rcode)     
         idvs = ncvdef(idDatFile,'s',NCFLOAT,2,dims,rcode) 
         idvT = ncvdef(idDatFile,'T',NCFLOAT,2,dims,rcode) 
...
!      CALL ncvpt(idDatFile,idvhxy, start, count(2), hxy, rcode)        
      ...
      CALL ncvpt(idDatFile,idvs, start, count, sprof, rcode) 
      CALL ncvpt(idDatFile,idvT, start, count, Tprof, rcode) 

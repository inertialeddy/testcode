...
#include "size.h"
 ...
 !---------------------
  ...
  REAL(kind=rc_kind) :: implvar=0.5d0                        ! Implicit code parameter. 0.5d0 is for Crank Nicholson
 ...
 !---------------------
   ...
   INTEGER, PARAMETER :: precip_begin =278, precip_end=378                                                                       
 ...
 !------------------------                                               
  ...
  REAL(kind=rc_kind)  :: yfront,yfront1,yfront2,yfront3,dyfront
...
!------------------------------------------------------------------------!
  ...
  REAL(kind=rc_kind), dimension(    0:NI+1,0:NJ+1, 0:NK+1)      :: zc,wx,wy,wz,p,strain,shear,Jac,Jacinv,rho,rho_old,rho_bar,rho_pri,vor,pv 
  ...
  REAL(kind=rc_kind), dimension(             NJ          )      :: stressx, stressy
  REAL(kind=rc_kind), dimension(    1:NI,1:NJ,1:NK,1:5   )      :: mat_A, mat_B, mat_C, mat_D
  REAL(kind=rc_kind), dimension(    1:NI,1:NJ,1:NK,1:5   )      :: mat_test    
  REAL(kind=rc_kind)                                            :: Hcontent, Hmixing
  REAL(kind=rc_kind), dimension(                   1:NK  )      :: salt_south,salt_north,tsouth,tnorth


  INTEGER                                                       :: selvar, solverlogic

  
! --- Sonaljit: I am defining wind stress component stressy here, instead of defining it locally in mixing_vertical routine
!  REAL(kind=rc_kind) :: stressy(NJ)
  ...
  REAL(kind=rc_kind), dimension(0:NJ+1)  ::  swr, qlatent, qloss, rainrate
  REAL(kind=rc_kind), dimension(0:NK+1)  ::  swrd
  REAL(kind=rc_kind), dimension(0:NJ+1,0:NK+1) :: swrflux
  REAL(kind=rc_kind) :: wA, lambda1, lambda2 ! Water type parameters. lambda are attenuation coefficients

 ! Tridiagonal matrix solution is stored in this array
  REAL(kind=rc_kind), dimension(1:NK) :: mat_ans
...
REAL(kind=rc_kind), dimension(1:NI,1:NJ,1:NK)::eps_shear,eps_buoy,eps_dest,D_eps
REAL(kind=rc_kind), dimension(1:NI,1:NJ,1:NK)::sgs_shear,sgs_buoy,sgs_dest,D_tke 
...
!!!! Assigning a integer for sign changing
  integer :: sign, kppcall, gotmlogic, kzlogic
...
REAL(kind=rc_kind), dimension(  0:NI+1,0:NJ+1,0:NK+1  ) :: KzMom, KzTr     

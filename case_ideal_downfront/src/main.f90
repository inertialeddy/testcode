...
#ifdef gotm_call
 gotmlogic = 1;
 if (pickup_step < -0.5) CALL initial_tke()
#endif
...
!       CALL initial_tke()
 ...
 if (pickup_step < -0.5) call write_cdf(step,0)
#endif
...
! OPEN(64, FILE='gotmtest/heatbudget.dat', STATUS='REPLACE')
...
!------HAVE ADDED THIS LINE TO UPDATE THE WIND-STRESS EVERY TIME-STEP---------------
  CALL fluxes(step) 
!--------------------------------------------


  CALL momentum(pcorr,step) 
!  write(64,*) step, Hcontent, Hmixing

!  OPEN(64, FILE,'heatbudget.dat', STATUS='REPLACE')
!  CALL heatbudget(64,step)

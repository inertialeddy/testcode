...
! !$OMP PARALLEL DO PRIVATE(istart,i,ii,im1,ip1,k,rh,subd,supd,dia,info) SHARED(p,cp,fn,nxm,nym,nzm) SCHEDULE(STATIC,CHUNK)

!$OMP PARALLEL DO PRIVATE(istart,i,ii,im1,ip1,k,rh,subd,supd,dia,info) SHARED(p,cp,fn,nxm,nym,nzm)
do j=1,nym 

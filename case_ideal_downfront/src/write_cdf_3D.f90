...
!----------------------------------------------------------             
  ...
  USE header, only : NI,NJ,NK,xc,yc,zc,p,h,consump,T,s,rho,Tr,u,v,w,vor,pv,uf,vf,wf,Kz,KzMom,KzTr,psom_tke,psom_eps, conv,con100,nconsume,dirout,rc_kind,ntr
#include "netcdf.inc"                                                                        
  ...
  integer :: iddatfile, idigit, idudx, idudy, idudz, idvbysection, idvby, idvbz, idvcon, idvcy, idvbx, idvkzmom, idvkztr, idvpsom_tke, idvpsom_eps
  ...
  do j=0,NJ+1 
    do i=0,NI+1 
      ...
      KzMom(i,j,NK+1) = KzMom(i,j,NK) ; KzMom(i,j,0) = KzMom(i,j,1) ;
      KzTr(i,j,NK+1) = KzTr(i,j,NK) ; KzTr(i,j,0) = KzTr(i,j,1) ;
      psom_tke(i,j,NK+1) = psom_tke(i,j,NK) ; psom_tke(i,j,0) = psom_tke(i,j,1) ;      
      psom_eps(i,j,NK+1) = psom_eps(i,j,NK) ; psom_eps(i,j,0) = psom_eps(i,j,1) ;
...
!  idDatFile =  nccre(TRIM(dirout)//outname_data,NCCLOB,rcode)
  ...
  idDatFile =  nccre(TRIM(dirout)//outname_data, or(NCCLOB,NF_64BIT_OFFSET),rcode)

!nccre(outname,or(NCCLOB,NF_64BIT_OFFSET),rcode)
  ...
  idvkzmom = ncvdef(idDatFile,'KzMom',NCDOUBLE,3,dims,rcode)
  idvkztr = ncvdef(idDatFile,'KzTr',NCDOUBLE,3,dims,rcode)  
  idvpsom_tke = ncvdef(idDatFile,'psom_tke',NCDOUBLE,3,dims,rcode)
  idvpsom_eps = ncvdef(idDatFile,'psom_eps',NCDOUBLE,3,dims,rcode)
  ...
  CALL ncvpt(idDatFile,idvkzmom, start, count, KzMom, rcode)
  CALL ncvpt(idDatFile,idvkztr, start, count, KzTr, rcode)
  CALL ncvpt(idDatFile,idvpsom_tke, start, count, psom_tke, rcode)
  CALL ncvpt(idDatFile,idvpsom_eps, start, count, psom_eps, rcode)

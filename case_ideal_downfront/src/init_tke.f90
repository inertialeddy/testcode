...
!!! Initializing a windstress profile for tke
do i=1,NI
do j=1,NJ
   stx(i,j) = sqrt(stressx(j)*stressx(j) + stressy(j)*stressy(j) ) 
   if (j.eq.1) then
     stx(i,0)=stx(i,1)
   endif
   if (j.eq.NJ) then
...
stx(0,0)=stx(1,1)
stx(0,NJ+1)=stx(1,NJ)
stx(NI+1,NJ+1)=stx(NI,NJ)
stx(NI+1,0)=stx(NI,1)

write(6,*) 'Wind at one wall is',   stx(NI/2,0)
write(6,*) 'Wind at another wall is',   stx(NI/2,NJ+1)
write(6,*) 'Wind at the center is',   stx(NI/2,NJ/2)

do i=0,NI+1
   do j=0,NJ+1
      u_fric(i,j) = (stx(i,j)/1000.d0)**0.5

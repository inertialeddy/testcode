...
do i=0,NI+1
do j=0,NJ+1
if (isnan(rho(i,j,k))) then
  print*, "rho is NaN at: Values ->", rho(i,j,k), T(i,j,k,n), s(i,j,k,n)
  print*, "At points: ",i,j,k
  STOP
end if
end do
end do 
...
if (isnan(rp(i,j,k))) then
  print*, "rp is NaN: Values ->", rho(i,j,k), T(i,j,k,n), s(i,j,k,n)
  print*, "At points: ",i,j,k
  STOP
end if
...
if (isnan(rp(i,j,k))) then
  print*, "rp is NaN here 2 ->",i,j,k, rho(i,j,k)
  STOP
end if
...
do i=0,NI+1
do j=0,NJ+1
if (isnan(rp(i,j,1))) then
  print*, "rp is NaN here 3 ->",i,j
  STOP
end if
end do
end do
...
if (isnan(grpifc(i,j,k) )) then
  print*, "rpevalgrad_Song 1: values are ->", i,j,k,rp(i+1,j,k),rp(i,j,k),gi(i,j,k,1)
  STOP
end if
...
if (isnan(grpifc(i,j,k) )) then
  print*, "rpevalgrad_Song 2: values are ->", rp(i,j+1,k),rp(i,j,k),gj(i,j,k,2)
  STOP
end if
